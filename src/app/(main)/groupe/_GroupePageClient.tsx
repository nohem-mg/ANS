                    ...
                    <div className="kf-bento-wrap">
                        ...
                        >
                            ...
                            <div className="kf-inner">
                                ...
                                <p className="kf-bento-fe-label">{featuredChiffre.label}</p>
                                <p className="kf-bento-fe-headline">{featuredChiffre.headline}</p>
                                <p className="kf-bento-fe-desc">{featuredChiffre.description}</p>
                            </div>
                            <span className="kf-bento-fe-bar" aria-hidden />
                        </motion.article>

                        <div className="kf-bento-cluster" role="list">
                            {smallFigures.map((fig, idx) => {
...
export default function GroupePageClient({ data }: { data: any }) {
    ...
    const featuredChiffre = {
        value: parseInt(data?.key_figures?.featured_value?.replace(/\s/g, '') ?? '60000'),
        suffix: '',
        label: data?.key_figures?.featured_label ?? FEATURED_CHIFFRE.label,
        headline: data?.key_figures?.featured_headline ?? FEATURED_CHIFFRE.headline,
        description: data?.key_figures?.featured_description ?? FEATURED_CHIFFRE.description,
    }

    const smallFigures = CHIFFRES_BENTO_PETITS.map((fallback, i) => ({
        ...fallback,
        value: parseInt(data?.key_figures?.small_figures?.[i]?.value?.replace(/\s/g, '') ?? String(fallback.value)),
        label: data?.key_figures?.small_figures?.[i]?.label ?? fallback.label,
        description: data?.key_figures?.small_figures?.[i]?.description ?? fallback.description,
    }))
                ...
                <div style={{ maxWidth: 960, margin: '0 auto' }}>
                    ...
                    <div className="adv-simple">
                        <ul className="adv-simple-list">
                            {advantageTitles.map((title: string, i: number) => (
                                <motion.li
                                    key={title}
