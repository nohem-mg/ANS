          ...
          >
            ...
            >
              ...
              >
                ...
                <button
                  onClick={() => setActiveStep(i => Math.min(stepsData.length - 1, i + 1))}
                  ...
                  style={{
                    ...
                    borderRadius: '50%',
                    border: 'none',
                    background: activeStep === stepsData.length - 1 ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.85)',
                    color: activeStep === stepsData.length - 1 ? 'rgba(43,18,0,0.3)' : '#2B1200',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    cursor: activeStep === stepsData.length - 1 ? 'default' : 'pointer',
                    boxShadow: '0 2px 12px rgba(0,0,0,0.2)',
                    transition: 'all 0.2s',
                    flexShrink: 0,
                  }}
...
export default function SolutionsPageClient({ data }: { data: any }) {
  ...
  const solutionsData = SOLUTIONS.map((fallback, i) => ({
    ...fallback,
  ...
  const galleryItems = solutionsData.map((s) => ({
    id: s.slug, title: s.title, description: s.summary,
    href: `/solutions/${s.slug}`, image: s.image,
  }))
